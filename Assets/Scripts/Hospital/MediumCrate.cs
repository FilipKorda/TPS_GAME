...
public class MediumCrate : MonoBehaviour
{
    private bool canOpen = false;
    public string description = "Press E to Open Crate";
    public string dontHaveDescription = "Nie masz m³ota i d³uta";
    public string dontHaveChDescription = "Nie masz d³uta";
    public string dontHaveHDescription = "Nie masz m³ota";
    ...
    private void Update()
    {
        if (canOpen && Input.GetKeyDown(KeyCode.E) && PickUpHospitalQuest.haveHammer && PickUpHospitalQuest.haveChisel)
        {
            if (countdownCoroutine == null)
            {

                AudioManager.Instance.PlaySFX("MediumCrateDestroying");
                descriptionText.text = "";
                countdownCoroutine = StartCoroutine(StartCountdown());
            }
        }
        else if (canOpen && !PickUpHospitalQuest.haveHammer && !PickUpHospitalQuest.haveChisel && Input.GetKeyDown(KeyCode.E))
        {
        ...
        {
            dontHaveDescriptionText.text = dontHaveHDescription;
            StartCoroutine(HideWhatYouGotText(1.5f));
        }
        else if (canOpen && PickUpHospitalQuest.haveHammer && !PickUpHospitalQuest.haveChisel && Input.GetKeyDown(KeyCode.E))
        {
            dontHaveDescriptionText.text = dontHaveChDescription;
            StartCoroutine(HideWhatYouGotText(1.5f));
        }
    }
    ...
    {
        AudioManager.Instance.PlaySFX("MediumCrateDestroy");
        Destroy(gameObject);
        DropObjectForPlayer();
    }
